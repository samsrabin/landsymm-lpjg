...
for d = 1:length(inDir_list)
    ...
    elseif strcmp_ignoreTrailSlash(inDir_list{d},'LPJGPLUM_1850-2010_PLUM6xtraMisc/matlab_merge_20180502104840') ...
        ...
        if contains(LUfile_tmp,'param "file_lu" (str "')
            ...
            LUfile = strrep(LUfile_tmp,'param "file_lu" (str "','') ;
            if isempty(dir(LUfile))
                error('That didn''t work: LUfile not found!')
            end
        else
        ...
        if contains(cropfile_tmp,'param "file_lucrop" (str "')
            ...
            cropfile = strrep(cropfile_tmp,'param "file_lucrop" (str "','') ;
            if isempty(dir(cropfile))
                error('That didn''t work: cropfile not found!')
            end
        else
    ...
    cropTypes(contains(cropTypes,{'CC3G_ic','CC4G_ic'})) = [] ;
    matches = [] ;
...
%                 disp('Regular:')
    ...
    if do_save.LU
        LUarea_ts_ntrl = getTS(LU,'NATURAL',land_area_YX) ;
        LUarea_ts_bare = getTS(LU,'BARREN',land_area_YX) ;
        LUarea_ts_crop = getTS(LU,'CROPLAND',land_area_YX) ;
        LUarea_ts_past = getTS(LU,'PASTURE',land_area_YX) ;
        save(timeseries_out,'LUarea_ts_ntrl','LUarea_ts_bare','LUarea_ts_crop','LUarea_ts_past',v73_or_append(timeseries_out)) ;
        if exist('LU0','var')
            LUarea_ts_crop0 = getTS(LU0,'CROPLAND',land_area_YX) ;
            LUarea_ts_past0 = getTS(LU0,'PASTURE',land_area_YX) ;
    ...
    if do_save.crops
        for c = 1:length(cropTypes)
            thisCrop = cropTypes{c} ;
            eval(['croparea_ts_' thisCrop ' = getTS(cropareas,thisCrop,ones(size(land_area_YX))) ;']) ;
        ...
        if exist('LU0','var')
            for c = 1:length(cropTypes)
                thisCrop = cropTypes{c} ;
                eval(['croparea0_ts_' thisCrop ' = getTS(cropareas0,thisCrop,ones(size(land_area_YX))) ;']) ;
...
%         end
        ...
        cpool_ts_VegC = getTS(cpool,'VegC',land_area_YX) * cf_cpool ;
        save(timeseries_out,'cpool_ts_VegC',v73_or_append(timeseries_out)) ;
        clear *_ts_*
        cpool_ts_LitterSoilC = getTS(cpool,{'LitterC','SoilC'},land_area_YX) * cf_cpool ;
        save(timeseries_out,'cpool_ts_LitterSoilC',v73_or_append(timeseries_out)) ;
        clear *_ts_*
        cpool_ts_HarvSlowC = getTS(cpool,'HarvSlowC',land_area_YX) * cf_cpool ;
        save(timeseries_out,'cpool_ts_HarvSlowC',v73_or_append(timeseries_out)) ;
        clear *_ts_*
        cpool_ts_Total = getTS(cpool,'Total',land_area_YX) * cf_cpool ;
...
%         end
        ...
        aiso_ts = getTS(aiso,'Total',land_area_YX) * cf_bvoc ;
        ...
        amon_ts = getTS(amon,'Total',land_area_YX) * cf_bvoc ;
    ...
    if do_save.Nflux
        ...
        nflux_ts_fert = getTS(nflux,'fert',land_area_YX) * cf_nflux ;
        save(timeseries_out,'nflux_ts_fert',v73_or_append(timeseries_out)) ;
        clear *_ts_*
        nflux_ts_flux = getTS(nflux,'flux',land_area_YX) * cf_nflux ;
        save(timeseries_out,'nflux_ts_flux',v73_or_append(timeseries_out)) ;
        clear *_ts_*
        nflux_ts_leach = getTS(nflux,'leach',land_area_YX) * cf_nflux ;
        save(timeseries_out,'nflux_ts_leach',v73_or_append(timeseries_out)) ;
        clear *_ts_*
        nflux_ts_harvest = getTS(nflux,'harvest',land_area_YX) * cf_nflux ;
        save(timeseries_out,'nflux_ts_harvest',v73_or_append(timeseries_out)) ;
        clear *_ts_*
        nflux_ts_LU_ch = getTS(nflux,'LU_ch',land_area_YX) * cf_nflux ;
    ...
    if do_save.Nfert    
        ...
        if isequal(sort(intersect(Nfert.varNames,{'CC3ann','CC3per','CC3nfx','CC4ann','CC4per'})),sort(Nfert.varNames))
            ...
            Nfert = CrOp_and_CrOpi(Nfert, 'Nfert', cropTypes, merge_or_replace) ;
        end
