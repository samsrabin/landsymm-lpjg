...
%%%%% Sam Rabin, 2018-10-25
% As v6, but with fix to mask of non-vegetated areas.
...
out_dir = '/Users/Shared/PLUM/input/remaps_v6.1/' ;
...
lu_out_XYyv = zeros([size(carea_XY) Nyears_out Nlu_out]) ;
for v = 1:Nlu_in
...
%     lu_out_XYy = PLUMharm_aggregate(lu_in_XYy.*carea_XYy,0.25,0.5)./carea_hd_XYy ;
    lu_out_XYy = lu_in_XYy ;
    if any(any(any(lu_out_XYy-1 > 1e-6)))
        error('Some element(s) of lu_out_XYy > 1!')
    end
    clear lu_in_XYy
    this_XYy = lu_out_XYyv(:,:,:,i) ;
    this_XYy = this_XYy + lu_out_XYy ;
    if any(any(any(this_XYy-1 > 2e-6)))
        error('Some element(s) of this_XYy > 1!')
    end
    lu_out_XYyv(:,:,:,i) = this_XYy ;
    clear lu_out_XYy
end
clear carea*_YXy
...
lu_out_XYyv(:,:,:,i) = lu_out_XYyv(:,:,:,i) ...
    + repmat(icwtr_XY,[1 1 Nyears_out]) ;
lu_out2_XYyv = zeros([size(carea_hd_XY) Nyears_out Nlu_out]) ;
for i = 1:Nlu_out
    disp(num2str(i))
    lu_out2_XYyv(:,:,:,i) = PLUMharm_aggregate(lu_out_XYyv(:,:,i).*carea_XYy,0.25,0.5)./carea_hd_XYy ;
end
...
out_file_lu = [out_dir 'LU.remapv6.1.txt'] ;
out_file_cropfrac = [out_dir 'cropfracs.remapv6.1.txt'] ;
out_file_nfert = [out_dir 'nfert.remapv6.1.txt'] ;
