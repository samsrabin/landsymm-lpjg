function out_YXv = PLUMharm_distMgmt(in_YXv,in_res,out_res)

res_ratio = in_res / out_res ;
if ~isint(log2(res_ratio))
    error('in_res / out_res must be a power of two!')
end

Ncrop = size(in_YXv,3) ;

out_YXv = nan([res_ratio res_ratio 1] .* size(in_YXv)) ;
for v = 1:Ncrop
    tmp_YX = in_YXv(:,:,v) ;
    for i = 1:log2(res_ratio)
        tmp_YX = interleave2(interleave2(tmp_YX,tmp_YX,'col'),interleave2(tmp_YX,tmp_YX,'col'),'row') ;
    end
    out_YXv(:,:,v) = tmp_YX ;
end
...
% March 4, 2014
%
% https://www.mathworks.com/matlabcentral/fileexchange/45757-interleave-vectors-or-matrices
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
