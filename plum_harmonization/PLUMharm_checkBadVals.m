...
if ~isempty(in_lu_YXv)
    ...
    elseif any(isBare) && any(any(vegd_YX==0 & landArea_YX>0))
        [i, j] = find(vegd_YX==0 & landArea_YX>0) ;
        warning('Zero vegetation in cell(s) with land! Max land area %0.1e, total %0.1e. E.g.: i=%d, j=%d', max(landArea_YX(vegd_YX==0)), sum(landArea_YX(vegd_YX==0)), i(1), j(1))
    end
end
...
if ~isempty(in_irrig_YXv)
    ...
    elseif max(in_irrig_YXv(:))>1+10^-outPrec
        warning('Value(s) >1 in %s irrig maps! Max overage %0.1e', msg, max(in_irrig_YXv(:))-1)
    end
end
