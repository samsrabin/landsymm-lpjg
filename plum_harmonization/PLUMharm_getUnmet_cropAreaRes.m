...
max_agri_y1_YXv = repmat(max_agri_y1_YX,[1 1 Nagri]) ;
...
if do_debug
    ...
    fprintf('      %s\t%0.4e\n',pad('unreserved:',20),unresArea_2deg_YX(dbI,dbJ)) ;
    fprintf('      %s\t%0.4e\n',pad('max agri:',20),max_agri_y1_YX(dbI,dbJ)) ;
end

% Check that no agri LU exceeds unreserved area. Unmet will need to go into
% agri LUs of neighboring cells.
% USING FIX TO MAKE UNMET POSITIVE
unmetA_YXv = (mid_y1_2deg_agri_YXv - max_agri_y1_YXv) .* (mid_y1_2deg_agri_YXv > max_agri_y1_YXv) ;
mid_y1_2deg_agri_YXv = mid_y1_2deg_agri_YXv - unmetA_YXv ;
if do_debug
    disp('   After check that no LU exceeds max agri area:')
...
% Check that sum(agri_y1) does not exceed max agri. area, and if it
...
% exceeds vegd, which means that agri *definitely* exceeds unreserved. It
...
while any(any(~isOK_YX))
    j = j + 1 ;
    if j>50
        error('Possible infinite loop in "Check that sum(agri_y1) does not exceed max agri. area."')
    end
