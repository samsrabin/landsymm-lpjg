...
in_y0_mgmtTot_YXv = (kg2mt*in_y0_mgmt_YXv) .* in_y0_area_YXv ;
in_y1_mgmtTot_YXv = (kg2mt*in_y1_mgmt_YXv) .* in_y1_area_YXv ;
out_y0_mgmtTot_YXv = (kg2mt*out_y0_mgmt_YXv) .* out_y0_area_YXv ;
out_y1_mgmtTot_YXv = (kg2mt*out_y1_mgmt_YXv) .* out_y1_area_YXv ;

mgmt_d_YXv = in_y1_mgmtTot_YXv - in_y0_mgmtTot_YXv ;

bad = 0 ;
for i = 1:length(LPJGcrops)
    mgmt_d2_YX = out_y1_mgmtTot_YXv(:,:,i) - out_y0_mgmtTot_YXv(:,:,i) + kg2mt*total_unmet_mgmt_YXv(:,:,i) ;
    ...
    if abs((mgmt_d_glob_2-mgmt_d_glob_1)/mgmt_d_glob_1*100) > conserv_tol_pct
        if sum(sum(out_y1_mgmtTot_YXv(:,:,i)))==0
            bad = -1 ;
