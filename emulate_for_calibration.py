...
def do_emulation(emulator_dir, GGCMIcrop, co2, t, w):

    # Irrigated
    KI = np.load('%s/%s_%s_I.npy'%(emulator_dir, GGCM, GGCMIcrop))
    ir_10  = emulate(KI, co2[decade], t[decade,:,:], 1, 10,  'NI', false)
    ir_60  = emulate(KI, co2[decade], t[decade,:,:], 1, 60,  'NI', false)
    ir_200 = emulate(KI, co2[decade], t[decade,:,:], 1, 200, 'NI', false)

    # Rainfed
    K  = np.load('%s/%s_%s.npy'%(emulator_dir, GGCM, GGCMIcrop))
    rf_10  = emulate(K,  co2[decade], t[decade,:,:],  w[decade,:,:], 10,  'N', false)
    rf_60  = emulate(K,  co2[decade], t[decade,:,:],  w[decade,:,:], 60,  'N', false)
    rf_200 = emulate(K,  co2[decade], t[decade,:,:],  w[decade,:,:], 200, 'N', false)
    return(rf_10, rf_60, rf_200, ir_10, ir_60, ir_200)


def do_emulation_wheats(emulator_dir, co2, ts, ws, tw, ww):
    rf_10w,rf_60w,rf_200w,ir_10w,ir_60w,ir_200w = do_emulation(emulator_dir, "winter_wheat", co2, tw, ww, false)
    rf_10s,rf_60s,rf_200s,ir_10s,ir_60s,ir_200s = do_emulation(emulator_dir, "spring_wheat", co2, ts, ws, false)
...
def PLUMemulate(GCM, GGCM, mask_YX, outarr_yield):
    ...
    for PLUMcrop in PLUMcrops:
        ...
        print("%s %s %s %s"%(GCM, GGCM, PLUMcrop, GGCMIcrop))
    ...
    return(outarr_yield, outheader, outfmt)
...
for GCM in GCMs:
    for GGCM in GGCMs:
        ...
        print("%s %s..."%(GCM, GGCM))
        outdir = '/project/ggcmi/AgMIP.output/Jim_Emulator/Sam/outputs_calib_%s/%s/%s/'%(datetime.datetime.now().strftime("%Y%m%d"), GCM, GGCM)
        ...
        # Emulate
        outarr_yield,outheader,outfmt = PLUMemulate(GCM, GGCM, mask_YX, lonlats, lonlats)
