...
% ggcm = 'GEPIC' ;
% crop = 'soy' ; crop_short = 'soy' ;
ggcm = 'pDSSAT' ;
crop = 'spring_wheat' ; crop_short = 'swh' ;
...
outDir = sprintf('%s/troubleshooting/moreFert_lessYield/%s_%s_A%d_N%d-N%d', ...
    topDir, ggcm, crop_short, adapt, Nlist(1), Nlist(2)) ;
if ~exist(outDir, 'dir')
    mkdir(outDir)
end
...
maps_YXint = nan(360, 720, 2, 2, Ntpers) ;
filenames_int = cell(2, 2, Ntpers) ;
for ii = 1:2
    thisIrr = irrList{ii} ;
    thisW = Wlist{ii} ;
    ...
    for n = 1:2
        thisN = Nlist(n) ;
        fprintf('Importing %s N%d...\n', thisIrr, thisN)
        
        % Baseline simulated
        t = 1 ;
%         filepattern = ['/Volumes/Reacher/GGCMI/AgMIP.output/GEPIC/phase2/soy/A%d/yield/' ...
%             'gepic_agmerra_fullharm_yield_soy_global_annual_1980_2010_C360_T0_W%s_N%d_A%d.nc4' ] ;
...
%         filename = sprintf(filepattern, adapt_ph2, thisW, thisN, adapt_ph2) ;
        ...
        filenames_int{ii,n,t} = filename ;
        maps_YXint(:,:,ii,n,t) = flip(permute(ncread(filename, varName), [2 1 3]), 1) ;
...
for t = 1:Ntpers
    ...
    tiledlayout('flow', ...
     ...
     for ii = 1:2
        ...
        map1_YX = maps_YXint(:,:,ii,1,t) ;
        map2_YX = maps_YXint(:,:,ii,2,t) ;
        ...
        if skip1
            ...
            caxis(new_caxis)
            colormap(gca, 'jet'); colorbar('Location', cbarLoc)
        ...
        if skip1 || skip2
            ...
            caxis(new_caxis)
            colormap(gca, new_cmap); colorbar('Location', cbarLoc)
            title(strrep(sprintf('%s: N%d - N%d', ...
                thisIrr, Nlist(2), Nlist(1)), ...
                '_', '\_'))
            set(gca, 'FontSize', fontSize)
        end
    end
    ...
    yN = tsN_list(t) ;
    if t == 1
        hsgt = sgtitle(strrep(sprintf('%s %s A%d (sim): %d-%d', ...
            ggcm, crop, adapt_ph2, y1, yN-1), ...
            '_', '\_')) ;
        filename = sprintf('%s/%s_%s_A%d_N%d-N%d_%d-%d_sim.png', ...
            outDir, ggcm, crop_short, adapt, Nlist(1), Nlist(2), y1, yN-1) ;
    else
        hsgt = sgtitle(strrep(sprintf('%s %s A%d (emu): %d-%d', ...
            ggcm, crop, adapt, y1, yN), ...
            '_', '\_')) ;
        filename = sprintf('%s/%s_%s_A%d_N%d-N%d_%d-%d_emu.png', ...
            outDir, ggcm, crop_short, adapt, Nlist(1), Nlist(2), y1, yN) ;
    end
