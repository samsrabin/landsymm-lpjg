...
% % %     continue
    ...
    thisVar_lpj = data_fu_out.varNames{v} ;
    thisCrop_lpj = getbasename(thisVar_lpj) ;
    thisN = thisVar_lpj(regexp(thisVar_lpj,'\d\d\d$'):end) ; % regexp: Get last 3 digits (must also be last 3 characters)
    thisI = strrep(strrep(thisVar_lpj,thisCrop_lpj,''), thisN, '') ;
%     fprintf('%s = %s + %s + %s\n', thisVar_lpj, thisCrop_lpj, thisI, thisN)
    thisCrop_emu = cropList_lpj_asEmu{strcmp(cropList_lpj, thisCrop_lpj)} ;
    thisVar_emu = sprintf('%s%s%s', thisCrop_emu, thisI, thisN) ;
    
    ii = find(strcmp(data_bl_lpj.varNames, thisVar_lpj)) ;
    if length(ii) ~= 1
        error('Error finding %s in data_bl_lpj.varNames (%d found)', thisVar_lpj, length(ii)) ;
    end
    tmp_i_lpj(v) = ii ;
...
%     % TROUBLESHOOTING
%     fprintf('%s <-- %s (ind emu %d)\n', thisVar_lpj, thisVar_emu, tmp_i_emu(v)) ;
end
if length(tmp_i_lpj) ~= length(unique(tmp_i_lpj))
    error('length(tmp_i_lpj) ~= length(unique(tmp_i_lpj))')
end
...
a = data_fu_out.garr_xvt(:,strcmp(data_fu_out.varNames,'CerealsC4200'),4) ./ ...
    data_bl_lpj.garr_xv(:,strcmp(data_bl_lpj.varNames,'CerealsC4200'),:) ;
...
title(sprintf('LPJ BL: %s (max %0.3f)', varName, max(y_lpj_bl)))

% h2 = subplot(3,1,2) ;
% hist(y_lpj_fu);
% xlims = [min(xlims(1), h2.XLim(1)) max(xlims(2), h2.XLim(2))] ;
% title(sprintf('LPJ %d-%d: %s (max %0.3f)', y1, yN, varName, max(y_lpj_fu)))

h2 = subplot(3,1,2) ;
ygt0 = y_lpj_fu(y_lpj_fu>0) ;
