...
thisVer = '20180730' ;
% Like 20180426, but puts setaside crop area into CC3G or CC4G.
...
PLUMtoLPJG{strcmp(LPJGcrops,'CC3G')} = {'setaside'} ;
PLUMtoLPJG{strcmp(LPJGcrops,'CC4G')} = {'setaside'} ;
...
%     copyfile(thisScript,['"' outDir '"']) ;
    ...
    for y = 1:Nyears
        ...
        if any(cropfracs_in_sum==0 & cropland_tmp>0)
            ...
            warning('NOT ACTUALLY SETTING TO ZERO');%landcover_in_tmp.CROPLAND(arebad) = 0 ;
            cropland_tmp = landcover_in_tmp.CROPLAND ;
...
%         wheat_isWW = -1 * ones(Ncells,1) ;
        for c = 1:Ncfts_lpjg
            ...
            if strcmp(thisLPJGcrop,'CC3G')
                thisSum(assign_cc4g) = 0 ;
                thisSum(assign_5050) = 0.5*thisSum(assign_5050) ;
            elseif strcmp(thisLPJGcrop,'CC4G')
                thisSum(assign_cc3g) = 0 ;
                thisSum(assign_5050) = 0.5*thisSum(assign_5050) ;
            end
    ...
    % Swap CC3Gi and CC4Gi to rainfed and remove, because these have no 
    % irrigated area.  If in the future PLUM (or something else) specifies
    % irrigation for  these, remove this part.
    cropfracs_in_Xvy(:,[find(strcmp(LPJGcrops_mid,'CC3G')) find(strcmp(LPJGcrops_mid,'CC4G'))],:) = ...
        cropfracs_in_Xvy(:,[find(strcmp(LPJGcrops_mid,'CC3Gi')) find(strcmp(LPJGcrops_mid,'CC4Gi'))],:) ;
    cropfracs_in_Xvy(:,contains(LPJGcrops_mid,{'CC3Gi','CC4Gi'}),:) = [] ;
    if save_nfert
        nfert_in_Xvy(:,[find(strcmp(LPJGcrops_mid,'CC3G')) find(strcmp(LPJGcrops_mid,'CC4G'))],:) = ...
            nfert_in_Xvy(:,[find(strcmp(LPJGcrops_mid,'CC3Gi')) find(strcmp(LPJGcrops_mid,'CC4Gi'))],:) ;
        nfert_in_Xvy(:,contains(LPJGcrops_mid,{'CC3Gi','CC4Gi'}),:) = [] ;
    end
    if save_irr
        irrig_in_Xvy(:,[find(strcmp(LPJGcrops_mid,'CC3G')) find(strcmp(LPJGcrops_mid,'CC4G'))],:) = ...
            irrig_in_Xvy(:,[find(strcmp(LPJGcrops_mid,'CC3Gi')) find(strcmp(LPJGcrops_mid,'CC4Gi'))],:) ;
        irrig_in_Xvy(:,contains(LPJGcrops_mid,{'CC3Gi','CC4Gi'}),:) = [] ;
    end
    LPJGcrops_out = LPJGcrops_mid ;
    LPJGcrops_out(contains(LPJGcrops_out,{'CC3Gi','CC4Gi'})) = [] ;
