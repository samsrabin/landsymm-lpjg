function remap_ani_v02_child(gridname)
...
%
% See previous notes for remap_ani_v01, with the following changes:
% - Now a function.
...
inpaint_method = 4 ;
out_dir = sprintf( ...
    '/Users/Shared/PLUM/input/remaps_%s/%s/', ...
    remapVer, gridname) ;
...
gridlist_dir = '/Users/Shared/ani_gridlists' ;
gridlist_file = sprintf('%s/%s.txt', gridlist_dir, gridname) ;
tmp = lpjgu_matlab_readTable_then2map( ...
    gridlist_file, 'xres', 1/12, 'yres', 1/12, 'force_mat_nosave', true) ;
countries_ok_YX = tmp.mask_YX ;
...
tmp_5min_YX = nan(size(this_YX,1),size(countries_ok_YX,2)) ;
for i = 1:6
    tmp_5min_YX(:,i:6:size(countries_ok_YX,2)) = this_YX ;
end
clear this_YX
this_5min_YX = nan(size(countries_ok_YX)) ;
for i = 1:6
    this_5min_YX(i:6:size(countries_ok_YX,1),:) = tmp_5min_YX ;
end
clear tmp_5min_YX
...
% Kludge from 0.5 degree (30-minute) to 5 minute resolution
tmp_5min_YX = nan(size(manure2crop_hd_YX,1),size(countries_ok_YX,2)) ;
for i = 1:6
    tmp_5min_YX(:,i:6:size(countries_ok_YX,2)) = manure2crop_hd_YX ;
end
clear this_YX
manure2crop_5min_YX = nan(size(countries_ok_YX)) ;
for i = 1:6
    manure2crop_5min_YX(i:6:size(countries_ok_YX,1),:) = tmp_5min_YX ;
end
% Interpolate
...
out_file_gridlist = [out_dir 'gridlist.remap.' remapVer '.' gridname '.txt'] ;
out_file_lu = [out_dir 'LU.remap.' remapVer '.' gridname '.txt'] ;
out_file_cropfrac = [out_dir 'cropfracs.remap.' remapVer '.' gridname '.txt'] ;
out_file_nfert = [out_dir 'nfert.remap.' remapVer '.' gridname '.txt'] ;
...
dlmwrite(out_file_gridlist,[lons lats],'delimiter',' ','precision',['%.' num2str(outPrec_lonlat) 'f'])
