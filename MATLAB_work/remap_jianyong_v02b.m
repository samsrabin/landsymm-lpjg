...
% Get mask
res = 0.5 ;
lat_map = repmat(transpose((-90+res/2):res:(90-res/2)),[1 size(gridlist.mask_YX,2)]) ;
lon_map = repmat((-180+res/2):res:(180-res/2),[size(gridlist.mask_YX,1) 1]) ;
lats = lat_map(gridlist.list_to_map) ;
lons = lon_map(gridlist.list_to_map) ;
...
out_file_gridlist = [out_dir 'gridlist.remap.' remapVer '.txt'] ;
...
dlmwrite(out_file_gridlist,[lons lats],'delimiter',' ')
