...
% See previous notes for remap_v4, with the following changes:
% 
% In remap_v4, we put unhandledCrop and setAside area into CC3G or CC4G
% crops. Here in remap_v5, these go into ExtraCrop, which will behave as
% wheat with LPJ-GUESS deciding whether it should be winter or spring.
PLUMsetAside_frac = 0.103 ;
%
% AgMIP assumes zero setAside area but still gets a reasonable global value
% for total N applied. Thus, here we multiply AgMIP fertilization rates by 
% 1/(1-PLUMsetAside_frac), so that we still get the correct amount of total
% N applied.
%
% In remap_v5, we add nitrogen from manure (Zhang et al., 2017) to
% the AgMIP fertilizer rates. Manure N is assumed to be applied at the same
% rate across all crops.
%
% Finally, we now use inpaint_method 4 instead of 0.
inpaint_method = 4 ;
...
load('/Users/sam/Geodata/Manure_ZhangEtAl2017/zhangManure_1860to2014_hd.mat') ;
