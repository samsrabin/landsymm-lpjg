...
%%%%% Sam Rabin, 2018-11-29
% See previous notes for remap_v6, with the following changes:
% - Nfert and irrigation for each crop set to zero for cells without any 
%   area of that crop.
% - Only including NH and no CerealsC3(i), since this run is intended to
%   test how well numbers correspond without confounding December-January
%   issue.
% - Saves gridlist for just NH.
...
out_dir = '/Users/Shared/PLUM/input/remaps_v6p4/' ;
...
gridlist.mask_YX(1:180,:) = false ;
gridlist.list_to_map = find(gridlist.mask_YX) ;
...
[list_cropsCombined_out, in2out_keyCombined_frac, ...
    ...
    get_remapv2_keys(thisVer) ;
% Remove CerealsC3
cc3 = strcmp(list_cropsCombined_out,'CerealsC3') ;
list_ignore_frac = [list_ignore_frac ; in2out_keyCombined_frac{cc3}'] ;
in2out_keyCombined_frac(cc3) = [] ;
list_cropsCombined_out(cc3) = [] ;
% (end Remove CerealsC3)
...
out_file_lu = [out_dir 'LU.remapv6p4.txt'] ;
out_file_cropfrac = [out_dir 'cropfracs.remapv6p4.txt'] ;
out_file_nfert = [out_dir 'nfert.remapv6p4.txt'] ;
