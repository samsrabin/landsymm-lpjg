function [albedo_jan_YXy,albedo_jul_YXy] = ...
    get_albedo_nobare(fpc, snowdepth, LU, albedo_bs_map, land_area_YX, pftNames)
...
% Adjust for no barren
LU_cnp_YX1y = 1 - LU.maps_YXvy(:,:,strcmp(LU.varNames,'BARREN'),:) ;
if any(LU_cnp_YX1y==0)
    error('How have you not excluded this?')
end
LU_ntrl_YX1y = LU.maps_YXvy(:,:,strcmp(LU.varNames,'NATURAL'),:) ./ LU_cnp_YX1y ;
LU_crop_YX1y = LU.maps_YXvy(:,:,strcmp(LU.varNames,'CROPLAND'),:) ./ LU_cnp_YX1y ;
LU_past_YX1y = LU.maps_YXvy(:,:,strcmp(LU.varNames,'PASTURE'),:) ./ LU_cnp_YX1y ;
% LU_ntrl_YX1y = LU.maps_YXvy(:,:,strcmp(LU.varNames,'NATURAL'),:) ;
% LU_crop_YX1y = LU.maps_YXvy(:,:,strcmp(LU.varNames,'CROPLAND'),:) ;
% LU_past_YX1y = LU.maps_YXvy(:,:,strcmp(LU.varNames,'PASTURE'),:) ;


% Get cover maps
fpc_map_cr_YXy = squeeze(sum(fpc.maps_YXvy(:,:,is_cr,:),3) .* LU_crop_YX1y) ;
fpc_map_gr_YXy = squeeze(sum(fpc.maps_YXvy(:,:,is_gr_ntrl,:),3) .* LU_ntrl_YX1y ...
                       + sum(fpc.maps_YXvy(:,:,is_gr_past,:),3) .* LU_past_YX1y ...
                       ) ;
fpc_map_ev_YXy = squeeze(sum(fpc.maps_YXvy(:,:,is_ev,:),3) .* LU_ntrl_YX1y) ;
fpc_map_de_YXy = squeeze(sum(fpc.maps_YXvy(:,:,is_de,:),3) .* LU_ntrl_YX1y) ;
fpc_map_te_YXy = squeeze(sum(fpc.maps_YXvy(:,:,is_te,:),3) .* LU_ntrl_YX1y) ;
% fpc_map_bs_YXy = 1 - (fpc_map_cr_YXy + fpc_map_gr_YXy + fpc_map_ev_YXy +...
%                       fpc_map_de_YXy + fpc_map_te_YXy) ;
fpc_map_bs_YXy = squeeze( ...
    (1 - fpc.maps_YXvy(:,:,strcmp(fpc.varNames,'Natural_sum'),:)) .* LU_ntrl_YX1y ...
  + (1 - fpc.maps_YXvy(:,:,strcmp(fpc.varNames,'Crop_sum'),:))    .* LU_crop_YX1y ...
  + (1 - fpc.maps_YXvy(:,:,strcmp(fpc.varNames,'Pasture_sum'),:)) .* LU_past_YX1y ...
  ) ;
% fpc_map_bs_YXy = squeeze((1 - fpc.maps_YXvy(:,:,strcmp(fpc.varNames,'Total'),:)) .* LU_cnp_YX1y) ;
