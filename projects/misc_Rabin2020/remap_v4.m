...
%
% Here in remap_v4, we also move a fraction of *handled* crop area into
% CC3G or CC4G. This fraction corresponds to what PLUM assumes for setAside
% area.
%
% Also includes a fix for what to use for interpolated Nfert map of
% irrigated crops.
%
% If C3/C4 tied, assign based on latitude. Tropics (within �22.5� of
...
out_dir = '/Users/Shared/PLUM/input/remaps_v4/' ;
...
% Add CC3G and CC4G to represent unhandled crops and setAside
...
areatocropgrass_YX = map_ignore_area_YX + setaside_area_YX ;
croparea_cc3g_YX(assignPC3G_YX) = areatocropgrass_YX(assignPC3G_YX) ;
croparea_cc4g_YX(assignPC4G_YX) = areatocropgrass_YX(assignPC4G_YX) ;
% Mask pastNPP NaNs
...
% out_file_lu = [out_dir 'LU.remapv4.' thisVer '.cgFertIrr0.setaside' strrep(num2str(PLUMsetAside_frac),'.','') '.m' num2str(inpaint_method) '.txt'] ;
% out_file_cropfrac = [out_dir 'cropfracs.remapv4.' thisVer allRF_txt '.cgFertIrr0.setaside' strrep(num2str(PLUMsetAside_frac),'.','') '.m' num2str(inpaint_method) '.txt'] ;
out_file_nfert = [out_dir 'nfert.remapv4.' thisVer allRF_txt '.cgFertIrr0.setaside' strrep(num2str(PLUMsetAside_frac),'.','') '.m' num2str(inpaint_method) '.txt'] ;
