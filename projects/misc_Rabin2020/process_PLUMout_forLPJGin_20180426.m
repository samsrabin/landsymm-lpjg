...
lpjg_in = lpjgu_matlab_readTable('/Users/Shared/PLUM/trunk_runs/LPJGPLUM_expt1.1_2006-2100_PLUM6xtra_20180412171654/rcp26/2011-2015/yield.out.gz','dont_save_MAT',true,'verbose',false) ;
    ...
    for y = 1:Nyears
        ...
        if y==1
            ...
            inds_PLUMcrops_cropfracs = find(...
                ...
                & ~contains(details_cols,'setaside') ...
                ) ;
            ...
            setaside_present_thisDir = any(strcmp(details_cols,'setaside_A')) ;
            save_setaside_thisDir = false ;
            if save_setaside && setaside_present_thisDir
                save_setaside_thisDir = true ;
                setasidefrac_in_tmp = details_in_tmp(:,[1:2 find(strcmp(details_cols,'setaside_A'))]) ;
            end
        ...
        if exist('setasidefrac_in_tmp','var')
            setasidefrac_in_tmp(remove_these_cells,:) = [] ;
        end
        ...
        if y==1
            ...
            if save_setaside_thisDir
                setasidefrac_in_Xvy = nan(Ncells,1,Nyears) ;
            end
        end
...
%         cropfracs2_in_tmp = table2array(cropfracs2_in_tmp(:,4:end)) ;
        ...
        if save_setaside_thisDir
            setasidefrac_in_tmp = table2array(setasidefrac_in_tmp(:,3)) ;
            setasidefrac_in_Xv = setasidefrac_in_tmp ;
        end
...
%                 wheat_isWW(thisSum>0 & cropfracs2_in_tmp.TeWWirr>0) = 1 ;
        ...
        if setaside_present_thisDir
            cropfracs_plusSetAside_in_sum_whereCrop = cropfracs_in_sum_whereCrop + setasidefrac_in_Xv(cropland_tmp>0) ;
            if any(abs(cropfracs_plusSetAside_in_sum_whereCrop - 1) > 0.01)
                error('Cropfrac difference too big to be comfortable! (including setaside)')
            end
        else
        ...
        if save_setaside_thisDir && any(isnan(setasidefrac_in_tmp(:)))
            error('Some value of setasidefrac_in_tmp is NaN.')
        end
        ...
        if save_setaside_thisDir
            setasidefrac_in_Xvy(:,:,y) = setasidefrac_in_Xv ;
        end
    ...
    elseif save_setaside_thisDir && any(isnan(setasidefrac_in_Xvy(:)))
        error('Some value of setasidefrac_in_Xvy is NaN.')
    end
    ...
    if Nyears_xtra>0
        ...
        if save_setaside_thisDir
            setasidefrac_in_Xvy = cat(3,repmat(setasidefrac_in_Xvy(:,:,1),[1 1 Nyears_xtra]),setasidefrac_in_Xvy) ;
        end
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% Write out: Set-aside fraction %%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    if save_setaside_thisDir
        % Get header and formatSpec
        out_header_setaside = 'Lon Lat Year setaside' ;
        out_formatSpec_setaside = ['%4.2f %4.2f %4.0f %1.' num2str(outPrec_mgmtInputs) 'f'] ;
        out_formatSpec_setaside = [out_formatSpec_setaside '\n'] ;
        
        % Set up output files
        outFile_setaside = [outDir 'setasidefrac.txt'] ;
        if exist(strrep(outFile_setaside,'\ ',' '),'file') && ~force_overwrite
            error(['Outfile already exists! (' outFile_setaside ')']) ;
        end
        fid1_setaside = fopen(strrep(outFile_setaside,'\ ',' '), 'w') ;
        fprintf(fid1_setaside,'%s \n',outFile_setaside);
        
        disp('Saving set-aside fraction...')
        for c = 1:5%Ncells
            thisLon = lons(c) ;
            thisLat = lats(c) ;
            
            setaside_out = [repmat(thisLon,[Nyears+Nyears_xtra 1]) repmat(thisLat,[Nyears+Nyears_xtra 1]) [yearList_xtra yearList]' squeeze(setasidefrac_in_Xvy(c,:,:))] ;
            
            if rem(c-1,fclose_every)==0
                fid1_setaside = fopen(strrep(outFile_setaside,'\ ',' '), 'a+') ;
            end
            
            fprintf(fid1_setaside,out_formatSpec_setaside,setaside_out') ;
            
            if rem(c,fclose_every)==0 || c==Ncells
                fclose(fid1_setaside) ;
            end
        ...
        disp('gzipping...')
        if exist([strrep(outFile_setaside,'\ ',' ') '.gz'],'file')
            unix(['rm ' outFile_setaside '.gz'])
        end
        unix(['gzip ' outFile_setaside]) ;
        disp('Done.')
    end
