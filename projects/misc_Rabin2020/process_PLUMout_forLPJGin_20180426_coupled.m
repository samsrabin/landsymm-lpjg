...
%
% SSR 2018-06-06
% Derived from process_PLUMout_forLPJGin_20180426.m
%
% USAGE
% #!/bin/bash
% INDIR="'/path/to/PLUM_outputs/'"        # Trailing slash can be left off
% OUTDIR="'/path/to/thisScript_outputs/'" # Trailing slash can be left off
% Y1=FIRST_PLUM_OUTPUT_YEAR
% YN=LAST_PLUM_OUTPUT_YEAR
% YSTEP=YEARS_INTERVAL_BETWEEN_PLUM_OUTPUTS
% Y1_PRE=inf     # If < y1, will repeat first PLUMout year for y1_pre:(y1-1)
% SOMEOFALL=TRUE # Ensure that each gridcell always has at least some tiny amount of every crop
% PREVIOUS_YIELD="'/path/to/previous_lpjg_outputs/yield.out.gz'"
% LUFILE="'/path/to/LUH2_LU_file.txt'"
% matlab -nodisplay -nodesktop -r "inDir=$INDIR; outDir=$OUTDIR; y1=$Y1; yN=$YN; yStep=$YSTEP; y1_pre=$Y1_PRE; someofall=$SOMEOFALL; previous_yield=$PREVIOUS_YIELD; LUfile=$LUFILE; run process_PLUMout_forLPJGin_20180426_coupled.m;"

% '/Users/Shared/PLUM/trunk_runs/LPJGPLUM_expt1.1_2006-2100_PLUM6xtra_20180412171654/rcp26/2011-2015/yield.out.gz'


% inDir = '/Volumes/WDMPP_Storage/Shared/PLUM/PLUM_outputs_for_LPJG/SSP1.v4.s1' ;
% outDir = '/Volumes/WDMPP_Storage/Shared/PLUM/PLUM_outputs_for_LPJG/SSP1.v4.s1.test' ;
% y1 = 2011 ;
% yN = 2015 ;
% yStep = 1 ;
% y1_pre = 2006 ;
...
addpath(addslashifneeded(scripts_dir))

% What should get saved?
save_gl = true ; % Save gridlist to output directory
save_lu = true ;
save_cropfracs = true ;
save_nfert = true ;
save_irr = true ;
save_yield = true ; % Save expected yields
...
LPJGcrops = {'CerealsC3';
    ...
    'StarchyRoots';
    'Miscanthus'} ;
LPJGcrops_out = [strcat(LPJGcrops,'i') ; LPJGcrops] ;
Ncfts_plum = length(PLUMcrops) ;
Ncfts_lpjg = length(LPJGcrops) ;
Ncfts_lpjg_out = 2*Ncfts_lpjg ;
PLUMtoLPJG = {} ;
...
lpjg_in = lpjgu_matlab_readTable(previous_yield,'dont_save_MAT',true,'verbose',false) ;
...
for y = 1:Nyears
    ...
    for c = 1:Ncfts_lpjg
        ...
        if strcmp(thisLPJGcrop,'CerealsC3w')
            ...
            %                 wheat_isWW(thisSum>0 & cropfracs2_in_tmp.TeWWirr==0) = 0 ;
            %                 wheat_isWW(thisSum>0 & cropfracs2_in_tmp.TeWWirr>0) = 1 ;
    ...
    lpj_lumap.maps_YXvy = double(lpj_lumap.maps_YXvy) ;   % Cluster has this as single for some reason?
    isMatch = abs(sum(lpj_lumap.maps_YXvy,3)-1) > 10^-(outPrec_LC+2) ;
    ntries = 0 ;
    tmp = sum(lpj_lumap.maps_YXvy,3)-1 ;
    while any(isMatch(:))
        ntries = ntries + 1 ;
        if ntries > 5
            error('Too many tries!')
        end
        lpj_lumap.maps_YXvy = lpj_lumap.maps_YXvy ./ repmat(sum(lpj_lumap.maps_YXvy,3),[1 1 size(lpj_lumap.maps_YXvy,3) 1]) ;
        tmp = sum(lpj_lumap.maps_YXvy,3)-1 ;
        isMatch = abs(tmp) > 10^-(outPrec_LC+2) ;
        clear tmp
    end

    % Deal with NATURAL/URBAN/BARREN
    if y==1
        disp('Getting year-1 info...')
        barren_lpj_map = lpj_lumap.maps_YXvy(:,:,strcmp(lpj_lumap.varNames,'BARREN'),end) ;
        vegd_lpj_map = sum(lpj_lumap.maps_YXvy(:,:,contains(lpj_lumap.varNames,{'NATURAL','CROPLAND','PASTURE'}),end),3) ;
    ...
    disp('Dealing with NATURAL/URBAN/BARREN...')
    ...
    vegd_old = sum(landcover_in_tmp(:,find(contains(landcover_cols,{'NATURAL','CROPLAND','PASTURE'}))-3),2) ;
    ...
    disp('Done with NATURAL/URBAN/BARREN.')
...
%%%%%[~,not_in_lpjg] = setdiff([lons lats],[lons_lpjg lats_lpjg],'rows','stable') ;
if exclude_nonlpjg_cells
    landcover_in_Xvy(not_in_lpjg,:,:) = [] ;
    cropfracs_in_Xvy(not_in_lpjg,:,:) = [] ;
    if save_irr
        irrig_in_Xvy(not_in_lpjg,:,:) = [] ;
    end
    if save_nfert
        nfert_in_Xvy(not_in_lpjg,:,:) = [] ;
    end
    if save_yield
        yield_in_Xvy(not_in_lpjg,:,:) = [] ;
    end
    lons(not_in_lpjg) = [] ;
    lats(not_in_lpjg) = [] ;
    Ncells = length(lons) ;
...
if save_gl
    ...
    if exist(outFile_gridlist,'file') && ~force_overwrite
        error(['Outfile already exists! (' outFile_gridlist ')']) ;
    end
    out_formatSpec_gridlist = '%4.2f %4.2f\n' ;
    fid1_gridlist = fopen(outFile_gridlist,'w') ;
    fprintf(fid1_gridlist,out_formatSpec_gridlist,[lons_4gl(rdmsam) lats_4gl(rdmsam)]') ;
    fclose(fid1_gridlist) ;
    fid1_gridlist = fopen(outFile_gridlist,'a+') ;
...
if save_lu
    ...
    fid1_landcover = fopen(outFile_landcover, 'w') ;
    ...
    for c = 1:Ncells
        ...
        landcover_out = [repmat(thisLon,[Nyears+Nyears_xtra 1]) repmat(thisLat,[Nyears+Nyears_xtra 1]) [yearList_xtra yearList]' shiftdim(landcover_in_Xvy(c,:,:))'] ;
        
        if rem(c-1,fclose_every)==0
            fid1_landcover = fopen(outFile_landcover, 'a+') ;
        end
    ...
    if do_gzip
        disp('gzipping...')
        if exist([outFile_landcover '.gz'],'file')
...
if save_cropfracs
    ...
    for c = 1:Ncells
        ...
        cropfracs_out = [repmat(thisLon,[Nyears+Nyears_xtra 1]) repmat(thisLat,[Nyears+Nyears_xtra 1]) [yearList_xtra yearList]' shiftdim(cropfracs_in_Xvy(c,:,:))'] ;
    ...
    if do_gzip
        disp('gzipping...')
        if exist([outFile_cropfracs '.gz'],'file')
...
if save_nfert
    ...
    for c = 1:Ncells
        ...
        nfert_out = [repmat(thisLon,[Nyears+Nyears_xtra 1]) repmat(thisLat,[Nyears+Nyears_xtra 1]) [yearList_xtra yearList]' shiftdim(nfert_in_Xvy(c,:,:))'] ;
    ...
    if do_gzip
        disp('gzipping...')
        if exist([outFile_nfert '.gz'],'file')
...
if save_irr
    ...
    if exist(outFile_irrig,'file') && ~force_overwrite
        error(['Outfile already exists! (' outFile_irrig ')']) ;
    end
    fid1_irrig = fopen(outFile_irrig, 'w') ;
    ...
    for c = 1:Ncells
        ...
        irrig_out = [repmat(thisLon,[Nyears+Nyears_xtra 1]) repmat(thisLat,[Nyears+Nyears_xtra 1]) [yearList_xtra yearList]' shiftdim(irrig_in_Xvy(c,:,:))'] ;
        
        if rem(c-1,fclose_every)==0
            fid1_irrig = fopen(outFile_irrig, 'a+') ;
        end
    ...
    if do_gzip
        disp('gzipping...')
        if exist([outFile_irrig '.gz'],'file')
...
if save_yield
    ...
    if exist(outFile_yield,'file') && ~force_overwrite
        error(['Outfile already exists! (' outFile_yield ')']) ;
    end
    fid1_yield = fopen(outFile_yield, 'w') ;
    ...
    for c = 1:Ncells
        ...
        yield_out = [repmat(thisLon,[Nyears+Nyears_xtra 1]) repmat(thisLat,[Nyears+Nyears_xtra 1]) [yearList_xtra yearList]' shiftdim(yield_in_Xvy(c,:,:))'] ;
        
        if rem(c-1,fclose_every)==0
            fid1_yield = fopen(outFile_yield, 'a+') ;
        end
    ...
    if do_gzip
        disp('gzipping...')
        if exist([outFile_yield '.gz'],'file')
