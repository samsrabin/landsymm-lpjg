function [data_fu_lpj, data_bl_emu, data_fu_emu, list2map] = ...
    e2p_align_gridlists(data_fu_lpj, data_bl_emu, data_fu_emu)

if ~isequal(data_bl_emu.list2map, data_fu_emu.list2map)
    error('Mismatch between baseline and future gridlists: Emulator')
end

% Skip all this if gridlists already align
if isequal(data_fu_lpj.list2map, data_fu_emu.list2map)
    list2map = data_fu_lpj.list2map ;

else
    
    [list2map, gridlist_IA, gridlist_IB] = intersect(data_fu_lpj.list2map, data_fu_emu.list2map) ;
